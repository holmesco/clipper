...
class SDDConeVar():
    ...
    def __init__(self, mdl, dim, name="sdd"):
        ...
        self.hvec = fu.Expr.vstack(self.generate_var())
...
def dd_cone_project(M, options=None, cone='DD', verbose=False):
    """Solve the homogenized dual problem using different cone approximations"""
    # Get options
    if options is None:
        TOL = 1e-5
    ...
    with fu.Model("Project") as mdl:
        ...
        t = mdl.variable(1)
        v = H.hvec - mat2hvec(M)
        mdl.constraint("C1", fu.Expr.vstack(t, v), fu.Domain.inQCone())
        mdl.objective(fu.ObjectiveSense.Minimize, t)

        if verbose:
            mdl.setLogHandler(sys.stdout)
            mdl.writeTask(f"proj_{cone}.ptf")
...
def prune_affinity(affinity, clique_size_lb):
    ...
    keep_inds = np.array(list(G_pruned.nodes))

    return affinity[keep_inds][:, keep_inds], keep_inds
